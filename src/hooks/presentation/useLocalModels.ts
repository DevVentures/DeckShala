...
async function fetchOllamaModels(): Promise<ModelInfo[]> {
  try {
    ...
    // Ollama not available, return empty array
    return [];
  }
}
...
async function fetchLMStudioModels(): Promise<ModelInfo[]> {
  try {
    ...
    // LM Studio not available, return empty array
    return [];
  }
}
...
export function getSelectedModel(): {
  ...
  try {
    ...
    logger.debug("Getting selected model from localStorage", { selected });
    return selected ? JSON.parse(selected) : null;
  } catch (error) {
    logger.error("Error getting selected model from localStorage", error as Error);
    return null;
  }
}
...
export function setSelectedModel(modelProvider: string, modelId: string): void {
  try {
    ...
    logger.debug("Saved model to localStorage", { data });
  } catch (error) {
    logger.error("Error saving model to localStorage", error as Error);
  }
}
