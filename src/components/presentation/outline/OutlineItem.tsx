...
export const OutlineItem = memo(function OutlineItem({
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "group flex items-center gap-4 rounded-md bg-muted p-4 transition-all hover:bg-muted/80 hover:shadow-sm",
    ...
    >
      ...
      <span className="min-w-[1.5rem] text-primary font-semibold">{index}</span>
      ...
      <button
        onClick={() => onDelete(id)}
        className="text-muted-foreground opacity-0 transition-all hover:text-red-500 hover:scale-110 group-hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 rounded p-1"
        aria-label="Delete slide"
        title="Remove this slide from outline"
      >
        <X size={20} />
      </button>
    </div>
