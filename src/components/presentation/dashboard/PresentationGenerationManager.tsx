...
import { getEnhancedPromptAction } from "@/app/_actions/presentation/template-actions";
...
export function PresentationGenerationManager() {
  ...
  useEffect(() => {
    logger.debug("Processing outline messages", {
      count: outlineMessages.length,
    });
    ...
    const startOutlineGeneration = async (): Promise<void> => {
      if (shouldStartOutlineGeneration) {
        try {
          ...
          logger.error("Error starting outline generation", error as Error, {
            presentationInput,
          });
  ...
  useEffect(() => {
    if (presentationCompletion) {
      try {
        ...
        logger.error("Error processing presentation XML", error as Error);
  ...
  useEffect(() => {
    if (shouldStartPresentationGeneration) {
      ...
      // Get enhanced prompt with template and brand kit
      (async () => {
        try {
          ...
          if (currentPresentationId) {
            const enhancedResult = await getEnhancedPromptAction(
              currentPresentationId,
              presentationInput ?? "",
            );
            ...
            if (enhancedResult.success) {
              ...
              if (enhancedResult.brandTheme) {
                logger.info("Brand theme applied to presentation generation", {
                  presentationId: currentPresentationId,
                  templateCategory: enhancedResult.templateCategory,
                });
              }

              logger.info("Using enhanced prompt for generation", {
                presentationId: currentPresentationId,
                hasTemplate: !!enhancedResult.templateCategory,
                hasBrandKit: !!enhancedResult.brandTheme,
          ...
          void generatePresentation(finalPrompt, {
            body: {
          ...
          logger.error("Failed to get enhanced prompt", error as Error);
          // Fallback to regular prompt if enhanced prompt fails
