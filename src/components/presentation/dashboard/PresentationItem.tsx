...
export function PresentationItem({
  ...
  const { mutate: deletePresentationMutation, isPending: isDeleting } =
    useMutation({
      ...
      onError: (error) => {
        logger.error("Failed to delete presentation", error as Error, {
  ...
  const { mutate: renameMutation, isPending: isRenaming } = useMutation({
    ...
    onError: (error) => {
      logger.error("Failed to rename presentation", error as Error, {
        presentationId: presentation.id,
      });
      toast({
        variant: "destructive",
  ...
  const { mutate: duplicateMutation, isPending: isDuplicating } = useMutation({
    ...
    onError: (error) => {
      logger.error("Failed to duplicate presentation", error as Error, {
        presentationId: presentation.id,
      });
      toast({
        variant: "destructive",
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      logger.debug("Presentation content check", {
        presentationId: presentation.id,
        hasContent:
          Object.keys(response?.presentation?.content ?? {}).length > 0,
      });
      ...
      logger.error("Failed to navigate to presentation", error as Error, {
        presentationId: presentation.id,
      });
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <div
        className={cn(
          "group relative flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-all hover:bg-accent/5 hover:shadow-md hover:scale-[1.01]",
