...
import { getTemplateInfoAction } from "@/app/_actions/presentation/template-actions";
...
export default function PresentationPage() {
  ...
  // Fetch template and brand kit info
  const { data: templateInfo } = useQuery({
    queryKey: ["template-info", id],
    queryFn: async () => {
      const result = await getTemplateInfoAction(id);
      if (result.success) {
        return result;
      }
      return null;
    },
    enabled: !!id && !isGeneratingPresentation,
  });
  ...
  const debouncedThemeUpdate = useCallback(
    debounce((presentationId: string, newTheme: string) => {
      updatePresentationTheme(presentationId, newTheme)
        .then((result) => {
          if (!result.success) {
            logger.error("Failed to update theme", new Error(result.message), {
              presentationId,
              newTheme,
            });
          }
        ...
        .catch((error) => {
          logger.error("Error updating theme", error as Error, {
            presentationId,
            newTheme,
          });
        });
  ...
  useEffect(() => {
    ...
    if (presentationData) {
      ...
      // Apply brand kit theme if available
      if (templateInfo?.brandKit) {
        const brandTheme = BrandKitService.applyBrandKitToTheme(
          templateInfo.brandKit,
        );
        logger.info("Applied brand kit theme to presentation", {
          presentationId: id,
          brandKitId: templateInfo.brandKit.organizationName,
        });

        // Set CSS variables for brand colors/fonts
        if (brandTheme) {
          ...
          if (brandTheme.colors?.secondary) {
            root.style.setProperty(
              "--brand-secondary",
              brandTheme.colors.secondary,
            );
          }
          if (brandTheme.typography?.heading) {
            root.style.setProperty(
              "--brand-heading-font",
              brandTheme.typography.heading,
            );
          }
          if (brandTheme.typography?.body) {
            root.style.setProperty(
              "--brand-body-font",
              brandTheme.typography.body,
            );
          }
        }
      }
      ...
      if (presentationData?.presentation?.theme) {
        ...
        if (themeId in themes) {
          ...
          void getCustomThemeById(themeId)
            ...
            .catch((error) => {
              logger.error("Failed to load custom theme", error as Error, {
                themeId,
              });
