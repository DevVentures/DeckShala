...
export const CHART_TYPES = {
  ...
  // Charts using name/size data structure
  HIERARCHICAL_CHARTS: [TREEMAP_CHART_ELEMENT],
  // Charts using multi-series data structure
  MULTI_SERIES_CHARTS: [COMPOSED_CHART_ELEMENT],
} as const;
...
export function areChartTypesCompatible(
  ...
  const isHierarchical1 = CHART_TYPES.HIERARCHICAL_CHARTS.includes(
    chartType1 as (typeof CHART_TYPES.HIERARCHICAL_CHARTS)[number],
  );
  const isHierarchical2 = CHART_TYPES.HIERARCHICAL_CHARTS.includes(
    chartType2 as (typeof CHART_TYPES.HIERARCHICAL_CHARTS)[number],
  );
  const isMultiSeries1 = CHART_TYPES.MULTI_SERIES_CHARTS.includes(
    chartType1 as (typeof CHART_TYPES.MULTI_SERIES_CHARTS)[number],
  );
  const isMultiSeries2 = CHART_TYPES.MULTI_SERIES_CHARTS.includes(
    chartType2 as (typeof CHART_TYPES.MULTI_SERIES_CHARTS)[number],
  );

  return (
    (isLabelValue1 && isLabelValue2) ||
    (isCoordinate1 && isCoordinate2) ||
    (isHierarchical1 && isHierarchical2) ||
    (isMultiSeries1 && isMultiSeries2)
  );
}
...
export function isChartType(elementType: string): boolean {
  return [
    ...
    ...CHART_TYPES.HIERARCHICAL_CHARTS,
    ...CHART_TYPES.MULTI_SERIES_CHARTS,
  ].includes(
    elementType as
    | (typeof CHART_TYPES.LABEL_VALUE_CHARTS)[number]
    | (typeof CHART_TYPES.COORDINATE_CHARTS)[number]
    | (typeof CHART_TYPES.HIERARCHICAL_CHARTS)[number]
    | (typeof CHART_TYPES.MULTI_SERIES_CHARTS)[number],
  );
}
