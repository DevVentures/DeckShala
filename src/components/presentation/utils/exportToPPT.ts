...
export class PlateJSToPPTXConverter {
  ...
  private calculateContentArea(slide: PlateSlide): {
    ...
    switch (slide.layoutType) {
      ...
      case "background":
        // Background image spans full slide, content uses full area with semi-transparent overlay
        return baseArea;
      default:
        return baseArea;
    }
  ...
  private async addRootImage(rootImage: RootImage, layoutType?: string) {
    if (!this.currentSlide) {
      logger.error("Cannot add root image: current slide is null");
      return;
    }
    if (!rootImage.url) {
      logger.warn("Root image has no URL, skipping image addition");
      return;
    }
    ...
    // Adjust image position based on layout type
    switch (layoutType) {
      case "left":
        imageOptions = {
          ...imageOptions,
      ...
      case "background":
        ...
        break;
      default:
        // Full slide image for undefined or unknown layout types
        logger.debug(`Unknown layout type: ${layoutType}, using full slide`);
        break;
    }
    ...
    // Apply sizing according to PptxGenJS documentation
    if (
      ...
      switch (objectFit) {
        case "contain":
          // Fit entire image within area while preserving aspect ratio
          imageOptions.sizing = {
            type: "contain",
            w: imageOptions.w,
            h: imageOptions.h,
          ...
          break;
        case "cover":
          // Fill entire area while preserving aspect ratio (may crop)
          imageOptions.sizing = {
            type: "cover",
            w: imageOptions.w,
            h: imageOptions.h,
          ...
          break;
        case "fill":
          // Stretch to fill area (no sizing property needed)
          break;
        case "scale-down":
          // Behave as contain if image is larger, otherwise use original size
          imageOptions.sizing = {
            type: "contain",
            w: imageOptions.w,
            h: imageOptions.h,
          ...
          break;
        default:
          // Use crop with improved positioning calculations
          // Convert 0-1 position to actual crop offset
          // Position represents the focal point of the image
          const cropX = Math.max(0, (objectPosition.x - 0.5) * imageOptions.w * 0.5);
          const cropY = Math.max(0, (objectPosition.y - 0.5) * imageOptions.h * 0.5);
    ...
    try {
      ...
      logger.debug("Successfully added root image", { layoutType, objectFit });
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      logger.error("Failed to add root image to slide", err, {
        imagePath,
        layoutType,
        imageOptions: {
          x: imageOptions.x,
      ...
      throw new Error(`Image addition failed: ${err.message}`);
    }
  }
  ...
  private async addImage(
    ...
    const height = 2; // Default image height in inches
    ...
    if (!measureOnly && imageUrl && this.currentSlide) {
      try {
        ...
        // Apply sizing with improved calculations
        switch (objectFit) {
          case "contain":
            // Fit entire image within area while preserving aspect ratio
            imageOptions.sizing = {
          ...
          case "cover":
            ...
            break;
          case "fill":
            // Stretch to fill area (no sizing property)
            break;
          case "scale-down":
            // Behave as contain if image is larger
            imageOptions.sizing = {
              type: "contain",
              w: width,
              h: height,
            ...
            break;
          default:
            // Use crop with improved positioning
            // Convert 0-1 focal point to crop offset
            const cropX = Math.max(0, (objectPosition.x - 0.5) * width * 0.5);
            const cropY = Math.max(0, (objectPosition.y - 0.5) * height * 0.5);
        ...
        this.currentSlide.addImage(imageOptions);
        logger.debug("Successfully added inline image", { x, y, width, height, objectFit });
      } catch (error) {
        const err = error instanceof Error ? error : new Error(String(error));
        logger.error("Failed to add inline image to slide", err, {
        });
        // Don't throw - continue rendering other elements
      }
    }
  ...
  private async addSVGToSlide(
    ...
    try {
      ...
      logger.warn("Failed to add SVG", { error });
