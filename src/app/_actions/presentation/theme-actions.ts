...
export async function createCustomTheme(formData: ThemeFormData) {
  try {
    ...
    logger.error("Failed to create custom theme", error as Error, { name: formData.name });
...
export async function updateCustomTheme(
  ...
  try {
    ...
    logger.error("Failed to update custom theme", error as Error, { themeId });
  ...
  try {
    ...
    if (existingTheme.logoUrl) {
      try {
        ...
        logger.error("Failed to delete theme logo", deleteError as Error, { themeId, logoUrl: existingTheme.logoUrl });
    ...
    logger.error("Failed to delete custom theme", error as Error, { themeId });
    return {
      success: false,
      message:
...
export async function getUserCustomThemes() {
  try {
    ...
    logger.error("Failed to fetch custom themes", error as Error);
    return {
      success: false,
...
export async function getPublicCustomThemes() {
  try {
    ...
    logger.error("Failed to fetch public themes", error as Error);
    return {
      success: false,
      message:
...
export async function getCustomThemeById(themeId: string) {
  try {
    ...
    logger.error("Failed to fetch theme", error as Error, { themeId });
    return {
      success: false,
