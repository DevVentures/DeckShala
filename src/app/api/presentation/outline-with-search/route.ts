...
export async function POST(req: Request) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          suggestion: "Please ensure Ollama is running and models are downloaded."
        },
        { status: 503 },
      );
    }

    // Prepare tools - only include webSearch if available
    const tools = search_tool ? { webSearch: search_tool } : undefined;
    ...
    logger.error("Error in outline generation with search", error as Error);
    return NextResponse.json(
      {
