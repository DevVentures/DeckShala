...
import { SlidesGenerationSchema, rateLimiter, sanitizeForXML } from "@/lib/validation";
...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as SlidesRequest;

    // Validate input
    let validatedData;
    try {
      validatedData = SlidesGenerationSchema.parse(body);
    ...
    // Sanitize inputs for XML
    const sanitizedTitle = sanitizeForXML(title);
    const sanitizedPrompt = sanitizeForXML(userPrompt);
    ...
    logger.error("Error in presentation generation", error as Error);
    return NextResponse.json(
      {
