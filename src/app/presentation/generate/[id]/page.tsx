...
export default function PresentationGenerateWithIdPage() {
  ...
  const clearPresentationCookie = () => {
    ...
    const domain =
      window.location.hostname === "localhost" ? "localhost" : ".deckshala.com";
  ...
  useEffect(() => {
    ...
    if (isGeneratingOutline && !generationStarted.current) {
      logger.debug("Starting outline generation after navigation", {
        presentationId: id,
      });
    ...
    if (presentationData && !isLoadingPresentation && !isGeneratingOutline) {
      ...
      if (presentationData.presentation?.searchResults) {
        try {
          ...
          logger.error("Failed to parse search results", error as Error, {
            presentationId: id,
          });
          setSearchResults([]);
        }
